...
namespace AbstractSchoolView
{
    public partial class FormReportMovingPdf : Form
    {
        ...
        private void ButtonMake_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var dataSource = logic.GetSchoolSupplies(dateTimePickerFrom.Value.Date, dateTimePickerTo.Value.Date);
        ...
        private void ButtonToPdf_Click(object sender, EventArgs e)
        {
            using (var dialog = new SaveFileDialog { Filter = "pdf|*.pdf" })
            {
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        logic.SaveSchoolSuppliesToPdfFile(new ReportBindingModel
                        {
